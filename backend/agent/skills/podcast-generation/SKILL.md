...
- Synthesize speech audio using **DashScope Qwen3-TTS**
- Mix audio chunks into a final podcast **WAV** file
...
python /mnt/skills/public/podcast-generation/scripts/generate.py \
  ...
  --output-file /mnt/user-data/outputs/generated-podcast.wav \
...
- `--output-file`: Absolute path to output **WAV** file (required)
...
python /mnt/skills/public/podcast-generation/scripts/generate.py \
  ...
  --output-file /mnt/user-data/outputs/ai-history-podcast.wav \
  --transcript-file /mnt/user-data/outputs/ai-history-transcript.md
```

This will generate:
- `ai-history-podcast.wav`: The audio podcast file
...
- Share both the podcast WAV and transcript MD with user using `present_files` tool
- Provide brief description of the generation result (topic, duration, hosts)
- Offer to regenerate if adjustments needed

## Requirements

Set **one** of these API keys (same as the rest of the app):

- `DASHSCOPE_API_KEY` or `QWEN_API_KEY`: Alibaba DashScope API key

Optional:

- `DASHSCOPE_API_BASE` / `QWEN_API_BASE`: API base (default Beijing `https://dashscope.aliyuncs.com/api/v1`)
- `QWEN_TTS_MODEL`: default `qwen3-tts-flash`
- `QWEN_TTS_VOICE_MALE` / `QWEN_TTS_VOICE_FEMALE`: system voice names (defaults: `Ethan`, `Cherry`)
...
- Long lines are split automatically to respect Qwen TTS per-request limits
