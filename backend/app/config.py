...
def _repo_root() -> Path:
    """Directory for ``config.yaml`` and optional ``.env`` (monorepo root or /app)."""
    here = Path(__file__).resolve()
    backend_app = here.parent
    backend_root = backend_app.parent
    monorepo_candidate = backend_root.parent
    if (monorepo_candidate / "backend" / "app").is_dir():
        return monorepo_candidate
    # Docker image or compose mount: only ``backend/`` at /app — config lives next to code.
    return backend_root
