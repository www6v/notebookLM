...
- 前端：在预览主图上叠层交互；悬停命中区域 → 视觉蒙版/描边；双击 → 将文本注入 `ChatPanel` 的输入框（与现有 `inputText` 行为一致）。
...
1. **加载**：当前页主图可用后，前端将 **与主图一致的图像字节** 提交 OCR（可带简单缓存键：`deckId + slideIndex`，避免来回切换重复请求）。
2. **进行中**：主图区域显示轻量提示（如顶部条或角标「正在识别版式…」）；失败时 Snackbar + 可重试（可选）。
...
| 文件 | 职责 |
|------|------|
| `SlideDeckPreviewDialog.vue` | 主图外包一层 `position: relative` 容器；叠 **透明交互层**（与图同显式尺寸）；请求 OCR、绘制高亮、处理 `mousemove` / `dblclick`。 |
...
| `zh-CN.ts` / `en` 等 | Snackbar 与提示文案（OCR 中、失败、只读无法注入等）。 |
...
- 防抖：切换 `selectedIndex` 或主图 URL 后 **300ms** 再请求，避免快速翻页打爆服务。
- 内存：同一 `deckId+index` 可缓存上一次 `regions`，关闭弹窗时清空。
...
1. 登录用户在笔记本中打开幻灯片预览，待 OCR 完成后，悬停在某段文字区域可见 **稳定高亮框**（与识别区域一致 order of magnitude）。
...
**文档版本**：2026-05-06  
**状态**：Spec（待评审通过后进入 implementation plan / 编码）
