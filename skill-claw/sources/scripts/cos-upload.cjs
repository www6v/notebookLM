...
// --- Upload via presigned PUT URL (NotebookLM; no signing) ---
function uploadPresigned(args) {
  const filePath = args.file;
  const presignedUrl = args['presigned-url'];
...
// --- Upload via PUT Object (IMA STS credentials) ---
function upload(args) {
  if (args['presigned-url']) {
    return uploadPresigned(args);
  }
...
function main() {
  ...
  const usePresigned = Boolean(args['presigned-url']);
  const missing = usePresigned
    ? ['file', 'presigned-url'].filter((k) => !args[k])
    : REQUIRED.filter((k) => !args[k]);
  if (missing.length) {
    console.error(`Missing required arguments: ${missing.map((k) => `--${k}`).join(', ')}`);
    console.error(
      `Usage: node cos-upload.cjs --file <path> [--presigned-url <url>] OR (--secret-id ... --cos-key ...) [--content-type <mime>] [--timeout <ms>]`,
    );
    process.exit(1);
  }
